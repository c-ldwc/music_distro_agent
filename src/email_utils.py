        ...
        if not self._creds or not self._creds.valid:
            if self._creds and self._creds.expired and self._creds.refresh_token:
                ...
                flow = InstalledAppFlow.from_client_secrets_file(
                    self.gmail_context.secret, self.gmail_context.scopes
                )
        ...
        for m in messages["messages"]:
            ...
            if "parts" in this_email["payload"]:
                for part in this_email["payload"]["parts"]:
                    ...
                    if (
                        "filename" in part
                        and part["filename"].endswith(".eml")
                        or part["mimeType"] == "message/rfc822"
                    ):
                        if "attachmentId" in part["body"]:
                            ...
                            eml = email.message_from_bytes(
                                bytes_eml, policy=policy.default
                            )
                                ...
                                if body:
                                    ...
                                    if eml.is_multipart():
                                        for sub_part in eml.walk():
                                            if (
                                                sub_part.get_content_type()
                                                == "text/plain"
                                            ):
                                                body_content = sub_part.get_content()
                                                break
                                    else:
                                        body_content = eml.get_payload(
                                            decode=True
                                        ).decode("utf-8")
                            ...
                            if body_content:
                                email_for_agent = boom_email(
                                    date=date, body=body_content
                                )
                                with open(
                                    self.email_dir / f"attach_{current_attach}.txt", "w"
                                ) as f:
            ...
            message_api.modify(
                userId=userID, id=m["id"], body={"removeLabelIds": ["UNREAD"]}
            ).execute()
